class Player {
  ...
  prepareNextTurn(warrior) {
    ...
    //walk to empty space
      ...
      nextTurn.debugMessage ='escape ' + escapeRoute;
      nextTurn.action = 'walk';
      nextTurn.direction = escapeRoute;
    }
    //walk to captives
    ...
    //detonate
      ...
      nextTurn.direction = scoutReport.enemiesMaxScore;
          ...
          getScanResult(warrior) {
            ...
            /* Comment */
            // const getCords = function(x,y) {return (Math.min( Math.abs(x - 1), Math.abs(y - 1) ) + ( Math.abs(y) % 4 ) * 2)}(1,0);
            return scan;
          }
          ...
          getScoutReport(scanResult, origin, previousLocations) {
            ...
            for (let surrondingLocation in allSurrondingLocations) {
              // if (!previousLocations.find(pLocation => pLocation[0] == allSurrondingLocations[surrondingLocation][0] && pLocation[1] == allSurrondingLocations[surrondingLocation][1])) {
                surrondingLocations[surrondingLocation] = allSurrondingLocations[surrondingLocation];
              // }
            }

            // const surrondingLocations = Object.values(this.getSurrondingLocations(origin)).filter(surrondingLocation => !previousLocations.includes(surrondingLocation));
            // const surrondingRanges = Object.values(surrondingLocations).filter(sLocation => !previousLocations.find(pLocation => pLocation[0] == sLocation[0] && pLocation[1] == sLocation[1]))
            ...
            if (previousLocations.length <= 2) {
              ...
              // scanResult.entities.filter(e => e.type === 'empty').map(entityLocation => entityLocation.location).filter(emptyLocation => !Object.values(this.getSurrondingLocations(location)).includes(emptyLocation))
              // !scanResult.arraysEqual(location, [0,0]) ? emptyLocationsAround.push([0,0]) : false;
            }
            ...
            for (let locationDirection in surrondingLocations) {
              ...
              for (let rangeDirection in surrondingRanges[locationDirection]) {
                if (surrondingRanges[locationDirection][rangeDirection][0] === origin[0] && surrondingRanges[locationDirection][rangeDirection][1] === origin[1]) {
                  // delete surrondingRanges[locationDirection][rangeDirection];
                }
              }
            }

            for (let direction in surrondingRanges) {
              //calculatning scores for each location in surronding locations
              ...
              for (let currentSurroundingSpace in surrondingRanges[direction]) {
                 //calculatning scores for each location in each surronding range

                 // for (let surroundingSpace in currentRange.surroundingSpaces) {
                   // const currentSurroundingSpace = currentRange.surroundingSpaces[surroundingSpace];
              ...
              if (previousLocations.length <= 2) {
                ...
                //   potentialLocations[this.getDirectionFromLocation([previousLocations[previousLocations.length - 2][0] - origin[0],previousLocations[previousLocations.length - 2][1] - origin[1]])] = this.getScoutReport(scanResult, previousLocations[previousLocations.length - 2], previousLocations);
                // }
                for (let emptyLocation in emptyLocationsAround) {
                  if (!scanResult.arraysEqual(Object.values(potentialLocations).map(pl => pl.location),emptyLocationsAround[emptyLocation])) {
                    ...
                    // potentialLocations[emptyLocationsAround[emptyLocation]] = this.getScoutReport(scanResult, emptyLocationsAround[emptyLocation], previousLocations);
                  }
                }
              }
            // for (let range in surrondingRanges) {
            //   let currentRange = {};
            //   currentRange[Object.keys(surrondingLocations)[range]] = this.getSurrondingLocations(surrondingLocations[Object.keys(surrondingLocations)[Number(range)]])[Object.keys(surrondingLocations)[range]];
            //
            //   // currentRange = this.getSurrondingLocations(surrondingLocations[currentRange.direction]);
            //
            //   enemiesScores[Object.keys(surrondingLocations)[range]] = 0;
            //   captivesScores[Object.keys(surrondingLocations)[range]] = 0;
            //
            //   if (enemies.find(enemy => scanResult.arraysEqual(enemy.location, currentRange[Number(range)]))) {
            //     enemiesScores[Object.keys(surrondingLocations)[range]] += 4;
            //   }
            //   if (captives.find(captive => scanResult.arraysEqual(captive.location, currentRange[Number(range)]))) {
            //     captivesScores[Object.keys(surrondingLocations)[range]] += 10;
            //     enemiesScores[Object.keys(surrondingLocations)[range]] = -20;
            //   }
            //   for (let surroundingSpace in currentRange.surroundingSpaces) {
            //     const currentSurroundingSpace = currentRange.surroundingSpaces[surroundingSpace];
            //
            //     if (enemies.find(enemy => scanResult.arraysEqual(enemy.location, currentSurroundingSpace))) {
            //       enemiesScores[Object.keys(surrondingLocations)[range]] += 2;
            //     }
            //     if (captives.find(captive => scanResult.arraysEqual(captive.location, currentSurroundingSpace))) {
            //       captivesScores[Object.keys(surrondingLocations)[range]] += 5;
            //       enemiesScores[Object.keys(surrondingLocations)[range]] = -20;
            //     }
            //   }
            // }
